...
#from cifar10_utils import load_data_cifar10
...
def train(model, data, args):
    ...
    dataAgumentation = True
    # callbacks
    log = callbacks.CSVLogger(args.save_dir + '/logMnist.csv')
    ...
    with open(args.save_dir + "model_Mnist.yaml", "w") as yaml_file:
        yaml_file.write(args.save_dir + model_yaml)
    # serialize weights to HDF5
    model.save_weights(args.save_dir + '/trained_model_Mnist.h5')
    print('Trained model saved to \'%s/trained_model_Mnist.h5\'' % args.save_dir)

    #from utils import plot_log
    #plot_log(args.save_dir + '/log_Fashion.csv', show=True)


    from utils import plot_log
    plot_log(args.save_dir + '/logMnist.csv', show=True)
...
def load_MNIST_fromFile( ):
    data = np.load('xtrainMnist.npy', allow_pickle=True)
...
if __name__ == "__main__":
    ...
    print("Fashion MNIST: x_train: ", x_train.shape, "x_test: ", x_test.shape)
